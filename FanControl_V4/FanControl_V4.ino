...
void setup() {
  ...
  _display.setRotation(2); //Rotate Screen 180°
  _display.setTextSize(2); //Bigger Text Size for Visibility
  ...
  digitalWrite(Temp_pin_Vcc, HIGH); // Pin 1 for Tempsensor High => Voltage Supply for Voltage Divider
}
...
void loop() {
  ...
  R_ADC_temp = R_ADC_temp / 1000;  // 1000 Reads for Smoothing


  for (i=0; i<70; i++){
    if ((R_ADC_temp <= temp_lookup[i]) && (R_ADC_temp >= temp_lookup[i+1]) ){
      temp = float(i) + (R_ADC_temp - temp_lookup[i]) / (temp_lookup[i+1] - temp_lookup[i]); //Reading Temperature from Lookup Table and linear Interpolation between the Points 
      break; //Ending Loop after Temperature is found
    }
  }

  PWM_fan = ((PWM_max_fan - PWM_min_fan)/(temp_max_fan - temp_min_fan))*(temp - temp_min_fan) + PWM_min_fan; //Calculation PWM-Signal Fan
  PWM_fan = constrain(PWM_fan, PWM_min_fan, PWM_max_fan); //Constraining Signal between min and max
  analogWrite(PWM_fan_pin, PWM_fan * 2.55); //Setting PWM-Signal and converting from percent -> 8Bit (100->255)

  PWM_pump = ((PWM_max_pump - PWM_min_pump)/(temp_max_pump - temp_min_pump))*(temp - temp_min_pump) + PWM_min_pump; //Calculation PWM-Signal Pump
  PWM_pump = constrain(PWM_pump, PWM_min_pump, PWM_max_pump); //Constraining Signal between min and max
  analogWrite(PWM_pump_pin, PWM_pump * 2.55); //Setting PWM-Signal and converting from percent -> 8Bit (100->255)

  // Display Refresh and Output
  _display.clearDisplay();
  _display.setCursor(0, 0);
  _display.print("T: ");
